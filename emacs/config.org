x#+TITLE: Tate's Emacs Configuration
...
Makes displaying line numbers the default. Toggle this with ~C-c l~ as defined under 'Keybindings'.
...
** Alternatively, Neotree for easy file navigation
I think I prefer Neotree overall, but treemac's lsp-symbols functionality is nice.
...
* Languages, tools, etc (I will now re-do this mess)
...
*** python-ts-mode (disabled for now)

This was causing some issues, so I'm returning to a more standard lsp.

#+begin_src emacs-none :results none
(use-package python-mode
  :straight t
...
;; force using treesitter by default
...
(add-hook 'prog-mode-hook #'my-eglot-format-buffer-on-save)
(add-hook 'python-ts-mode-hook #'my-eglot-format-buffer-on-save)
...
Elpy is a (not well-maintained) alternative I want to try;#
