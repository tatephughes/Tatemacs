...
#+begin_src emacs-lisp :results none
(setq org-latex-precompile-command "%l --shell-escape -output-directory %o -ini -jobname=%b \"&%L\" mylatexformat.ltx %f") ;; Need this for minted

(define-minor-mode org-auto-export-pdf-mode
  "Automatically export Org files to PDF on save."
  :lighter " OrgPDF"
  :global nil
  (if org-auto-export-pdf-mode
      (add-hook 'after-save-hook #'org-latex-export-to-pdf nil 'local)
    (remove-hook 'after-save-hook #'org-latex-export-to-pdf 'local)))

;; Bind a key to toggle the mode (e.g., C-c C-x p)
(global-set-key (kbd "<XF86Launch6>") 'org-auto-export-pdf-mode)
#+end_src
